...
# ========================
  ...
  it '16. returns the names of users who ordered one specific item' do
    expected_result = [@user_2.name, @user_1.name, @user_3.name]
...
# ========================
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(orders: :items).select("users.name, COUNT(items.id) AS total_item_count").group(:id).order("users.name")
    # ---------------------------------------------------------------
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    data = User.joins(orders: :order_items).select("users.name as user_name, orders.id as order_id, orders.amount / count(order_items.id) as avg_item_cost").group("users.name, orders.id").order("users.name DESC").order("avg_item_cost")
    # ---------------------------------------------------------------
